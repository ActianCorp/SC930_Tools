...
def EndQry(qtext, begin_ts, end_ts, nano_thresh, user_id, database_name):
    ...
    if dur > nano_thresh:
        # Work out earliest and latest query timestamps that we've seen
        if First_qry == 0:
        ...
        try:
            LRQ_list.append([qtext, begin_ts, end_ts, dur, dbmspid, sessid, user_id, database_name])
        except:
            if gui:
...
def FindLRQ(path, nano_thresh, Pwin, qryOnly):
    ...
    qtext = ''
    begin_ts = 0
    # Each file should contain only one session, so clear out the session context for each new file
    user_id = '<not known>'
    database_name = '<not known>'
    ...
    for line in fh.readlines():
        ...
        if rectype in SC930_QQRY:
            ...
            if not qryOnly:
                ...
                # Append the cursor id to Fetch and Close statements, so we can work out which one we're handling
                if rectype == "FETCH" or rectype == "CLOSE":
                    cursor_id = words[1].split(':')[1].rstrip('\n').lstrip()
                    qtext = qtext + ' cursor ' + cursor_id

                # Prepend the Cursorid to the start of the appropriate statement, so we can trace it back if needed
                if rectype == "ADD-CURSORID":
                    cursor_id = words[1].split(':')[1].replace(' ', '').rstrip('\n')
                    qtext = 'define cursor ' + cursor_id

        elif rectype in SC930_EQY:
            # for an EQY end the query
            end_ts = words[1].split(':')[0]
            if start_found:
                # user_id and database_name are passed from the last begin session records that were found
                # so will not be present if there were no such records
                if not EndQry(qtext, begin_ts, end_ts, nano_thresh, user_id, database_name):
        ...
        # for Begin session records - track the user id and database name
        elif rectype in SC930_BSESS:
            user_id = words[1].split('(')[2].rstrip(' )')
            database_name = words[1].split('(')[6].rstrip(' )')

        # for Begin session records - track the user id and database name
        elif rectype in SC930_ESESS:
            # When we find the end of a session, blank out the session-context fields so they are not accidentally
            # carried over if no new records are found
            user_id = '<not known>'
            database_name = '<not known>'
...
def cli_main(argv=sys.argv):
    ...
    parser = OptionParser(usage="%s [-nrqde] [-t time] [-c csvfilename] [file(s)]" % progname,
                          version="%s %s" % (progname, SC930_LRQ_VER))
    parser.add_option("-n", "--nosort", action="store_true",
                      dest="nosort", default=False,
                      help="do NOT sort results (default is sort longest duration to shortest)")
    parser.add_option("-r", action="store_true",
                      dest="revsort", default=False, help="reverse the query sort order")
    parser.add_option("-t", "--threshold",
                      dest="thresh", default=DEF_THRESH, type=float,
                      help="ignore query durations less than this, in seconds. Default %ss" % (DEF_THRESH))
    ...
    parser.add_option("-c", "--csvfile",
                      dest="csvfile", default=False,
                      help="output a formatted CSV file to the named file instead of stdout")
    parser.add_option("-d", "--begindateorder", action="store_true",
                      dest="dateorder", default=False, help="sort in begin timestamp query order")
    parser.add_option("-e", "--enddateorder", action="store_true",
                      dest="enddateorder", default=False, help="sort in end timestamp query order")

    (options, filelist) = parser.parse_args()

    # no sort order option is mutually exclusive with specific sort orders
    if options.nosort and (options.revsort or options.dateorder or options.enddateorder):
        print "-n and (-r -d -e) sort choices are mutually exclusive. No sorting will be used."
    ...
    if options.nosort:
        ...
        # Allow revsort to reverse the default order of any sort option
        sort_order = False
        if options.revsort:
        ...
        try:
            if options.dateorder:
                # Begin Tx timestamp is field 1
                LRQ_sorted = sorted(LRQ_list, key=lambda item: item[1], reverse=sort_order)
            elif options.enddateorder:
                # End tx timestamp is field 2
                LRQ_sorted = sorted(LRQ_list, key=lambda item: item[2], reverse=sort_order)
            else:
                # Default sort order is by duration, field 3
                LRQ_sorted = sorted(LRQ_list, key=lambda item: item[3], reverse=sort_order)
            LRQ_list = []
        except:
    ...
    if options.csvfile:
        try:
            f_obj = open(options.csvfile, 'wb')

            writer = csv.writer(f_obj, quoting=csv.QUOTE_MINIMAL)
            # Write the header row
            writer.writerow(['QueryText', 'BeginTimeStamp', 'EndTimeStamp', 'Duration', 'DBMS-PID', 'SessionId', 'UserId', 'DatabaseName'])

            # Sinplest output option: writer.writerows(LRQ_sorted)
            # However, we want a pretty-printed version of the time stamps and durations, so format each column
            for row in LRQ_sorted:
                writer.writerow([
                    row[0],
                    GetNiceTime(row[1]),
                    GetNiceTime(row[2]),
                    "%20.9f" % (float(row[3]) / NANO_PER_SEC),
                    row[4], row[5], row[6], row[7]
                ])
            f_obj.close()
        except:
            print ('Cannot write complete output to named CSV file. File not writable or filesystem full.')
            return
    else:
        # output the results to stdout if not producing the CSV file
